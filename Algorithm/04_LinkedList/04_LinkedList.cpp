...
int main()
{
	...
	// 면접 TIP
	/// 리스트는 임의 접근이 안됨.
	// 중간 삽입 삭제가 빠르다고 한 이유는 
	// 내가 실질적으로 삭제할 대상을 이렇게 들고 있어가지고 그 위치를 기억하고 있을때, 빠르다는 의미다.
	// 만약에 이렇게 들고 있는 형태가 아니라 그냥 우리가 리스트만 떡하니 들고 있는 상태에서는 전혀 안빠름.

	// 즉 우리가 실질적으로 삭제할 그 위치를 혹은 추가할 위치를 이렇게 저장하고 있을 때---> 그때만 빠르다.
